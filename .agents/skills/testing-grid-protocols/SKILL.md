...
- Amount semantics are surprising: `server/payment-gateways/mpesa.ts` already converts `Amount` to cents
  (`Amount * 100`) and `updatePaymentFromCallback` multiplies by 100 **again**, so a callback only matches a
  payment of `N` cents when the callback `Amount` is `N / 10000`. A realistic major-unit amount lands in the
  "Amount mismatch ... holding for reconciliation" branch (payment stays `pending`). Check whether this double
  conversion is intended before reading a mismatch as a harness error.
